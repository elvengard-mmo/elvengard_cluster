defmodule ElvenGard.Cluster.Testing.LocalCluster do
  @moduledoc """
  ...
  **ALL CREDIT TO**: https://github.com/whitfin/local-cluster
  ...
  def start() do
    ...
    start_boot_server = fn ->
      # init only once
      call_once("local_cluster:started", fn ->
  ...
  def start_nodes(prefix, amount, options \\ [])
    ...
    nodes = Enum.map(peer_nodes, &elem(&1, 1))
  ...
  defp call_once(key, callback) do
    :persistent_term.get(key)
  catch
    :error, :badarg ->
      :persistent_term.put(key, callback.())
  end
end
